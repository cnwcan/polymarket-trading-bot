...
import { primeBookTickerHandshakeSkew, startDashboard } from "./dashboard";
...
async function discoverMarket(api: PolymarketApi, asset: string, timeframe: string): Promise<Market> {
  ...
  const trySlug = async (slug: string): Promise<Market | null> => {
    try {
...
async function main(): Promise<void> {
  ...
  if (config.polymarket.polymarketProxyUrl) {
    setupProxy(config.polymarket.polymarketProxyUrl);
  }

  logPrintln("=== Polymarket 5m Scalper Bot ===");
  logPrintln(`Mode: ${simulation ? "SIMULATION" : "PRODUCTION"}`);
  logPrintln(`Timeframe: ${timeframe} (${duration}s)`);
  logPrintln(`Position: $${scalperConfig.minPositionUsdc}-$${scalperConfig.maxPositionUsdc}`);
  logPrintln(`Take profit: ${scalperConfig.takeProfitPercent * 100}% | Stop loss: ${scalperConfig.stopLossPercent * 100}%`);
  logPrintln(`Entry threshold: ${scalperConfig.entryThresholdPercent}% in ${scalperConfig.momentumWindowMs / 1000}s`);
  logPrintln(`Ask range: $${scalperConfig.minEntryAsk.toFixed(2)}-$${scalperConfig.maxEntryAsk.toFixed(2)}`);
  logPrintln(`Cooldown: ${scalperConfig.scalpCooldownSeconds}s | Max open: ${scalperConfig.maxOpenPositions}`);
  logPrintln(`Daily loss limit: $${scalperConfig.dailyLossLimit}`);
  logPrintln("");

  const api = new PolymarketApi(config.polymarket);

  if (!simulation) {
    logPrintln("Authenticating with Polymarket CLOB...");
    let authOk = false;
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        await api.authenticate();
        logPrintln("Authentication successful");
        authOk = true;
        break;
      } catch (e) {
        logPrintln(`[Auth] Attempt ${attempt}/3 failed: ${e}`);
        if (attempt < 3) {
          const delay = attempt * 3000;
          logPrintln(`[Auth] Retrying in ${delay / 1000}s...`);
          await new Promise(r => setTimeout(r, delay));
        }
      }
    }
    if (!authOk) {
      logPrintln("[Auth] FATAL: Authentication failed after 3 attempts. Cannot run in PRODUCTION mode without valid API credentials.");
      logPrintln("[Auth] Check PRIVATE_KEY, SIGNATURE_TYPE, and PROXY_WALLET_ADDRESS in .env");
      process.exit(1);
    }
    ...
    try {
      ...
      if (!ok) {
        logPrintln(
          `[Auth] FATAL: USDC allowance is 0 or insufficient (balance=$${balance.toFixed(2)}, allowance=$${allowance.toFixed(2)}). ` +
        ...
        logPrintln(`[Auth] Allowance check OK (balance=$${balance.toFixed(2)})`);
      }
    } catch (e) {
      logPrintln(`[Auth] Allowance check failed — proceeding with caution: ${e}`);
    }
  }

  const asset = config.trading.markets[0] ?? "btc";
  logPrintln(`Asset: ${asset.toUpperCase()}`);

  // Create feed objects and scalper early (dashboard needs references)
  const binance = new BinancePriceFeed(scalperConfig.binanceWsUrl);
  const polyFeed = new PolymarketOrderbookFeed();
  const scalper = new Scalper(api, binance, polyFeed, scalperConfig, simulation);
  ...
  primeBookTickerHandshakeSkew();

  // Connect Binance WebSocket
  binance.connect();
  binance.on("tick", () => {});
  logPrintln("Waiting for Binance price feed...");
  await new Promise<void>((resolve) => {
    const check = () => {
      if (binance.lastPrice > 0) resolve();
      else setTimeout(check, 500);
    };
    check();
  });
  logPrintln(`Binance connected. BTC/USDT: $${binance.lastPrice.toFixed(2)}`);

  // Discover initial market
  logPrintln(`Discovering ${asset.toUpperCase()} ${timeframe} market...`);
  let currentMarket = await discoverMarket(api, asset, timeframe);
  const monitor = new MarketMonitor(api, `${asset.toUpperCase()} ${timeframe}`, currentMarket);
  ...
  logPrintln("Waiting for Polymarket orderbook...");
  await new Promise<void>((resolve) => {
    const check = () => {
      if (polyFeed.bestAsks.size > 0) resolve();
      else setTimeout(check, 500);
    };
    check();
  });
  logPrintln("Polymarket orderbook connected");

  live.periodTs = MarketMonitor.extractTimestampFromSlug(currentMarket.slug);
  ...
  logPrintln("Bot ready — waiting for Start from dashboard\n");
  ...
  setInterval(() => {
    ...
    logPrintln(
      `[Status] BTC: $${binance.lastPrice.toFixed(0)} | ` +
  ...
  (async () => {
    for (;;) {
      ...
      logPrintln("[Main] Period ended — discovering new market...");
      ...
      for (let attempt = 0; attempt < 10; attempt++) {
        try {
          ...
          if (!newTokens.upTokenId || !newTokens.downTokenId) {
            logPrintln("[Main] Failed to resolve new tokens, retrying...");
          ...
          live.periodTs = MarketMonitor.extractTimestampFromSlug(newMarket.slug);
          await scalper.setMarket({
          ...
          logPrintln(`[Main] New period: ${newMarket.slug}`);
          break;
        } catch (e) {
