...
export class PolymarketApi {
  ...
  private readonly DEFAULT_FEE_RATE_BPS = 1000;
  ...
  private async getClobClient(): Promise<ClobClient> {
    ...
    try {
      ...
      throw new Error(
        `CLOB API key creation/derivation failed. Check PRIVATE_KEY, SIGNATURE_TYPE, PROXY_WALLET_ADDRESS and account permissions. Original error: ${String(e)}`
      );
    }
    if (!creds || !creds.key || !creds.secret || !creds.passphrase) {
      const raw = JSON.stringify(creds);
      throw new Error(
        `CLOB API key derivation returned invalid credentials (key/secret/passphrase missing). ` +
  ...
  async getCollateralBalance(): Promise<number> {
    ...
    return rawBal / 1_000_000;
  }

  async checkCollateralAllowance(): Promise<{ balance: number; allowance: number; ok: boolean }> {
    const client = await this.getClobClient();
    const bal = await client.getBalanceAllowance({
      asset_type: AssetType.COLLATERAL,
      signature_type: this.config.signatureType,
    ...
    const balance = (Number.isFinite(rawBal) && rawBal > 0) ? rawBal / 1_000_000 : 0;
    ...
    const allowance = (Number.isFinite(rawAllowance) && rawAllowance > 0) ? rawAllowance / 1_000_000 : 0;
    const ok = allowance > 0 && (allowance >= balance || allowance > 1_000_000);
  ...
  async placeMarketOrder(
    ...
    if (side === "BUY") {
      ...
      orderSize = takerAmt > 0 ? takerAmt / 1_000_000 : amount;
    } else {
    ...
    let resp: any;
    try {
      resp = await client.postOrder(signedOrder, OrderType.FOK);
    } catch (e) {
    ...
    const orderId = resp.orderID as string | undefined;
    ...
    if (status !== "matched") {
      if (orderId && (status === "live" || status === "delayed")) {
        try { await client.cancelOrder({ orderID: orderId } as any); } catch {}
        console.error(`[API] FOK returned status="${status}" — cancelled orphan order ${orderId}`);
      }
      throw new Error(
        `Order not filled (status="${status}"). FOK requires immediate full fill. ` +
  ...
  private extractErrorDetails(e: unknown): string {
    ...
    if (err.response && typeof err.response === "object") {
      ...
      if (resp.status) return `HTTP ${resp.status}: ${JSON.stringify(data ?? resp.statusText)}`;
    }
