...
import { BinancePriceFeed } from "./binanceWs";
import { PolymarketOrderbookFeed } from "./polymarketWs";
import type { ScalperConfig } from "./config";
import { logPrintln } from "./logger";
import { sendTradeAlert, type TradeAlertData } from "./telegram";
...
type ScalpPhase =
  ...
  | { kind: "Cooldown"; until: number; lastDirection: "Up" | "Down" };
...
export type TradeCallback = (data: {
  side: string; entryPrice: number; exitPrice: number; shares: number;
  costBasis: number; proceeds: number; pnlUsd: number; pnlPercent: number;
  exitReason: string;
}) => void;
...
export class Scalper {
  ...
  private polyFeed: PolymarketOrderbookFeed;
  private config: ScalperConfig;
  private simulation: boolean;

  private market: MarketInfo | null = null;
  private _marketGeneration = 0;
  private phase: ScalpPhase = { kind: "Idle" };
  private openPositionCount = 0;
  private dailyLoss = 0;
  private dailyProfit = 0;
  private totalProfit = 0;
  private dailyResetDate = this.todayStr();
  private tickInterval: ReturnType<typeof setInterval> | null = null;
  private paperTradeSeq = 0;
  private lastScalperDebugAt = 0;
  private sellRetryCount = 0;
  private readonly MAX_SELL_INNER_RETRIES = 3;
  private readonly MAX_SELL_STUCK_RETRIES = 5;
  private readonly BUY_FEE_BUFFER = 1.025;
  private minOrderSizeShares = 1;
  private isPlacingBuy = false;
  private isExitingPosition = false;
  private _tickRunning = false;
  private geoBlockedUntil = 0;
  private stuckSellShares: number | null = null;
  private onTradeCallback: TradeCallback | null = null;
  private _autoScale = false;
  private _autoScaleBaseMin = 1;
  private _autoScaleBaseMax = 5;
  private _startingBalance = 20;
  private liveStartingBalance: number | null = null;
  private liveCurrentBalance: number | null = null;
  private _lastPeriodicSyncAt = 0;
  private readonly PERIODIC_SYNC_INTERVAL_MS = 60_000;
  ...
  async setMarket(info: MarketInfo): Promise<void> {
    if (this.phase.kind === "InPosition") {
      ...
      logPrintln("[Scalper] Period rollover while in position — attempting sell before market switch");
    ...
    logPrintln(`[Scalper] Market set: ${info.slug} | Up: ${info.upTokenId.slice(0, 12)}... | Down: ${info.downTokenId.slice(0, 12)}...`);
  }
  ...
  setSimulationMode(sim: boolean): void {
    this.simulation = sim;
    logPrintln(`[Scalper] Mode switched to ${sim ? "SIMULATION" : "PRODUCTION"}`);
  }

  setPositionRange(min: number, max: number): void {
    const lo = Math.max(1, Math.min(min, max));
    const hi = Math.max(lo, Math.min(max, 100));
    this.config.minPositionUsdc = lo;
    this.config.maxPositionUsdc = hi;
    logPrintln(`[Scalper] Position range set: $${lo}-$${hi}`);
  }

  setMinOrderSizeShares(minShares: number): void {
    const v = Number(minShares);
    this.minOrderSizeShares = Number.isFinite(v) && v > 0 ? v : 1;
    logPrintln(`[Scalper] Market min order size: ${this.minOrderSizeShares} shares`);
  }
  ...
  setAutoScale(enabled: boolean, baseMin: number, baseMax: number): void {
    ...
    logPrintln(`[Scalper] Auto-scale ${enabled ? "ON" : "OFF"}: base $${this._autoScaleBaseMin}-$${this._autoScaleBaseMax}`);
  }

  private getEffectiveRange(): { min: number; max: number } {
    if (!this._autoScale) {
      return { min: this.config.minPositionUsdc, max: this.config.maxPositionUsdc };
    }
    const currentBalance =
      !this.simulation && this.liveCurrentBalance != null
  ...
  async syncStartingBalanceFromPolymarket(): Promise<number> {
    if (this.simulation) return this._startingBalance;
    try {
      ...
      logPrintln(`[Scalper] Live sync starting balance: $${this._startingBalance.toFixed(2)}`);
      return this._startingBalance;
    } catch (e) {
      logPrintln(`[Scalper] Live sync starting balance FAILED: ${e}`);
      return this._startingBalance;
    }
  }

  async syncCurrentBalanceFromPolymarket(): Promise<number> {
    if (this.simulation) return this._startingBalance + this.totalProfit;
    try {
      const bal = await this.api.getCollateralBalance();
      ...
      logPrintln(`[Scalper] Live sync current balance FAILED: ${e}`);
  ...
  start(): void {
    ...
    logPrintln("[Scalper] Started (500ms tick)");
  }
  ...
  private async _tickInner(): Promise<void> {
    ...
    if (this.isExitingPosition) return;
    ...
    if (this.dailyLoss >= this.config.dailyLossLimit) {
      this.maybeLogScalperDebug(
        `Durdu: günlük zarar limiti ($${this.dailyLoss.toFixed(2)} ≥ $${this.config.dailyLossLimit}).`
      );
      if (this.phase.kind === "InPosition") {
    ...
    if (this.phase.kind === "InPosition") {
      if (timeToClose <= this.config.exitBeforeCloseSeconds) {
        if (this.sellRetryCount > 0) {
          logPrintln(`[Scalper] Period ending while sell stuck — abandoning (retries won't help, market will resolve)`);
      ...
      if (this.sellRetryCount > 0 && !this.simulation) {
        if (this.sellRetryCount >= this.MAX_SELL_STUCK_RETRIES) {
          logPrintln(
            `[Scalper] SELL ABANDONED after ${this.sellRetryCount} stuck retries — position will resolve at market expiry. ` +
            `Accepting loss and moving on.`
          );
          await this.abandonCurrentPosition("sell_abandoned");
          return;
        }
        logPrintln(`[Scalper] Retrying stuck SELL (attempt ${this.sellRetryCount + 1}/${this.MAX_SELL_STUCK_RETRIES})`);
    ...
    if (this.openPositionCount >= this.config.maxOpenPositions) {
      this.maybeLogScalperDebug(
        `Giriş yok: açık pozisyon limiti (${this.openPositionCount}/${this.config.maxOpenPositions}).`
      );
      return;
    }
    ...
    if (timeToClose <= this.config.exitBeforeCloseSeconds + 15) {
      this.maybeLogScalperDebug(
        `Giriş yok: periyot sonuna çok yakın (${timeToClose}s ≤ ${this.config.exitBeforeCloseSeconds + 15}s tampon; yeni pozisyon açılmıyor).`
      );
      return;
    }
  ...
  private maybeLogScalperDebug(message: string): void {
    const iv = this.config.entryDebugLogIntervalMs;
    if (iv <= 0) return;
    const now = Date.now();
    if (now - this.lastScalperDebugAt < iv) return;
    this.lastScalperDebugAt = now;
    logPrintln(`[Scalper][debug] ${message}`);
  }
   ...
   */
  private async checkEntrySignal(): Promise<void> {
    if (!this.market) return;
    ...
    const snapshot = (): string => {
      ...
      return (
        `BTC~$${this.binance.lastPrice.toFixed(0)} | upAsk=$${upAsk.toFixed(2)} downAsk=$${downAsk.toFixed(2)} | ` +
    ...
    if (side && this.phase.kind === "Cooldown") {
      if (side === this.phase.lastDirection && Math.floor(Date.now() / 1000) < this.phase.until) {
        this.maybeLogScalperDebug(
          `Signal guard: ${side} yönünde cooldown aktif (${this.phase.until - Math.floor(Date.now() / 1000)}s kaldı). ${snapshot()}`
        );
        return;
      }
      this.phase = { kind: "Idle" };
    ...
    if (this.dailyLoss >= this.config.dailyLossLimit) {
      this.maybeLogScalperDebug(
        `Günlük zarar limitine ulaşıldı ($${this.dailyLoss.toFixed(2)} ≥ $${this.config.dailyLossLimit}). | ${snapshot()}`
      );
      return;
    }

    // --- Whale / spike protection (configurable via env) ---
    const SPIKE_WINDOW_MS = 5_000;
    ...
    if (btcSwing5s > SPIKE_THRESHOLD) {
      logPrintln(
        `[Scalper] WHALE GUARD: BTC spike ${(btcSwing5s * 100).toFixed(3)}% in ${SPIKE_WINDOW_MS / 1000}s ` +
    ...
    const spreadExpansion = this.polyFeed.getSpreadExpansion(targetTokenId, 10_000);
    if (spreadExpansion > SPREAD_EXPANSION_LIMIT) {
      logPrintln(
        `[Scalper] WHALE GUARD: ${side} spread expanded ${spreadExpansion.toFixed(1)}x vs median ` +
        `(limit ${SPREAD_EXPANSION_LIMIT}x) — liquidity vacuum, skipping entry`
      );
      return;
    }

    const sizing = this.computeSmartPositionSize(
      Math.abs(spotChange), threshold, entryPrice, side, upAsk, downAsk
    );
    ...
    if (shares < 1) {
      this.maybeLogScalperDebug(
        `Pay adedi < 1: ${shares.toFixed(2)} pay. Position=$${positionUsdc} | ${snapshot()}`
      );
      return;
    }
    ...
    if (shares < this.minOrderSizeShares) {
      ...
      if (candidateUsdWithBuffer <= maxAllowed + 1e-9) {
        ...
        this.maybeLogScalperDebug(
          `Min shares constraint prevents entry: need ${candidateShares} shares ($${candidateUsd.toFixed(2)}, ` +
    ...
    logPrintln(
      `[Scalper] ENTRY SIGNAL: ${side} | Spot Δ${windowSec}s: ${(spotChange * 100).toFixed(3)}% | ` +
  ...
  private async checkLateEntrySignal(timeToClose: number): Promise<void> {
    if (!this.market) return;
    ...
    if (!side || !tokenId || entryPrice <= 0) {
      this.maybeLogScalperDebug(
        `Late entry: no clear direction (upAsk=$${upAsk.toFixed(2)} downAsk=$${downAsk.toFixed(2)}, need >= $${lateMinAsk.toFixed(2)})`
      );
      return;
    }

    // Whale protection for late entries too
    const btcSwing5s = this.binance.getMaxSwing(5_000);
    if (btcSwing5s > 0.0015) {
      logPrintln(
        `[Scalper] WHALE GUARD (late): BTC spike ${(btcSwing5s * 100).toFixed(3)}% in 5s — skipping late entry`
      );
      return;
    }
    ...
    if (positionUsdc < eff.min) return;
    ...
    if (shares < this.minOrderSizeShares) {
      ...
      if (candidateUsd <= eff.max && candidateUsd <= availableBalance) {
        ...
        this.maybeLogScalperDebug(
          `Late entry: minOrderSize ${bufferedShares} shares ($${candidateUsd.toFixed(2)}) exceeds ` +
    ...
    if (minCost > availableBalance || minCost > eff.max) {
      this.maybeLogScalperDebug(
        `Late entry: cost $${minCost.toFixed(2)} exceeds limit (balance=$${availableBalance.toFixed(2)}, max=$${eff.max.toFixed(0)})`
      );
      return;
    }
    ...
    const entryRuleSummary =
      `LATE ENTRY ${timeToClose}s left: ${side} @ $${entryPrice.toFixed(2)} (${(impliedProb * 100).toFixed(0)}% implied) | ` +
      `Expected ROI: +${expectedReturn.toFixed(1)}% | Position: $${(shares * entryPrice).toFixed(2)}`;

    logPrintln(
      `[Scalper] LATE ENTRY: ${side} | Ask: $${entryPrice.toFixed(2)} | ` +
   ...
   */
  private computeSmartPositionSize(
    absSpotChange: number,
    threshold: number,
    entryPrice: number,
    side: "Up" | "Down",
    upAsk: number,
    downAsk: number
  ): { positionUsdc: number; confidence: number; kellyFraction: number; spreadQuality: number } {
  ...
  private async executeBuy(
    ...
    try {
      ...
      logPrintln(`[Scalper] BUY ${side} ${shares} shares @ $${price.toFixed(4)}`);

      let actualBuyCostUsdc: number | undefined;
      ...
      if (this.simulation) {
        ...
        try {
          ...
          if (bal < cost * 0.95) {
            logPrintln(`[Scalper] BUY skipped: insufficient balance ($${bal.toFixed(2)} < $${cost.toFixed(2)} cost)`);
            this.phase = { kind: "Cooldown", until: Math.floor(Date.now() / 1000) + 30, lastDirection: side };
            return;
          }
        } catch (e) {
          logPrintln(`[Scalper] Balance pre-check failed, proceeding: ${e}`);
        }
        ...
        for (let i = 0; i < maxAttempts; i++) {
          if (i > 0) {
            ...
            if (currentAsk > 0 && currentAsk > originalAsk * (1 + MAX_BUY_SLIPPAGE)) {
              logPrintln(
                `[Scalper] BUY RETRY ABORTED: ask moved $${originalAsk.toFixed(4)} → $${currentAsk.toFixed(4)} ` +
            ...
            try {
              ...
              if (estimatedCost > freshBal * 0.90) {
                logPrintln(
                  `[Scalper] BUY RETRY ABORTED: cost $${estimatedCost.toFixed(2)} > 90% balance $${(freshBal * 0.90).toFixed(2)}`
                );
                break;
              }
            } catch (e) {
              logPrintln(`[Scalper] Pre-retry balance check failed, proceeding: ${e}`);
            }
          }
          try {
            const result = await this.api.placeMarketOrder(tokenId, shares, "BUY");
            if (result.actualPrice !== undefined) fillPrice = result.actualPrice;
            if (result.actualSize !== undefined) fillShares = result.actualSize;
            logPrintln(`[Scalper] REAL: Buy order placed${i > 0 ? ` (attempt ${i + 1})` : ""} — fillPrice=$${fillPrice.toFixed(4)} fillShares=${fillShares}`);
            placed = true;
            break;
          } catch (e) {
            if (this.isGeoBlockedError(e)) {
              const mins = 15;
              this.geoBlockedUntil = Date.now() + mins * 60_000;
              this.phase = { kind: "Cooldown", until: Math.floor(this.geoBlockedUntil / 1000), lastDirection: side };
              logPrintln(`[Scalper] GEO-BLOCKED (403). Live order flow paused for ${mins} minutes. Switch to SIM mode or run from an allowed region.`);
              return;
            }
            logPrintln(`[Scalper] BUY FAILED (attempt ${i + 1}/${maxAttempts}): ${e}`);
          }
        }
        if (!placed) {
          logPrintln("[Scalper] BUY ABANDONED after 3 attempts — entering 60s cooldown");
        ...
        if (slippage > MAX_BUY_SLIPPAGE) {
          logPrintln(
            `[Scalper] SLIPPAGE REJECT: filled @ $${fillPrice.toFixed(4)} vs signal ask $${originalAsk.toFixed(4)} ` +
          ...
          try {
            ...
            if (bal > 0) {
              ...
              logPrintln(`[Scalper] SLIPPAGE reversal SELL placed (${bal.toFixed(4)} shares)`);
            }
          } catch (e) {
            logPrintln(`[Scalper] SLIPPAGE reversal SELL failed: ${e}`);
          }
          this.phase = { kind: "Cooldown", until: Math.floor(Date.now() / 1000) + 60, lastDirection: side };
          return;
        }
        ...
        try {
          ...
          if (verified) {
            logPrintln(
              `[Scalper] BUY FILL VERIFIED via Polymarket: ${verified.size} shares @ $${verified.price.toFixed(4)} ` +
            ...
            logPrintln(`[Scalper] BUY fill not found in Polymarket history (using order price)`);
          }
        } catch (e) {
          logPrintln(`[Scalper] BUY fill verification failed: ${e}`);
        }
        ...
        for (let attempt = 0; attempt < BALANCE_CHECK_DELAYS.length; attempt++) {
          ...
          try {
            ...
            logPrintln(`[Scalper] Post-buy balance check failed (attempt ${attempt + 1}): ${e}`);
          }
          if (actualBalance >= expectedShares) {
            logPrintln(
              `[Scalper] Token balance confirmed: ${actualBalance.toFixed(6)} shares` +
          ...
          if (actualBalance > 0 && actualBalance < expectedShares) {
            logPrintln(
              `[Scalper] Partial settlement: ${actualBalance.toFixed(6)} / ~${shares.toFixed(4)} expected — ` +
              `waiting for full settlement (attempt ${attempt + 1}/${BALANCE_CHECK_DELAYS.length})...`
            );
          } else if (actualBalance <= 0) {
            logPrintln(`[Scalper] Balance still 0 — waiting (attempt ${attempt + 1}/${BALANCE_CHECK_DELAYS.length})...`);
          }
        }

        if (actualBalance <= 0) {
          logPrintln(
            `[Scalper] BUY PHANTOM DETECTED: order reported success but token balance is 0 ` +
          ...
          try { await this.api.cancelAllOrders(); } catch {}
          this.phase = { kind: "Cooldown", until: Math.floor(Date.now() / 1000) + 60, lastDirection: side };
          return;
        }

        if (actualBalance < shares) {
          logPrintln(`[Scalper] Post-buy balance: ordered ${shares.toFixed(4)} → settled ${actualBalance.toFixed(6)} (fee/settlement adjustment)`);
          shares = actualBalance;
          cost = shares * price;
        }

        if (this._marketGeneration !== genAtStart) {
          logPrintln(`[Scalper] BUY completed but market changed during order — attempting immediate sell to avoid orphaned position`);
          try {
            await this.api.placeMarketOrder(tokenId, shares, "SELL", "0.01", false, 0.20);
            logPrintln(`[Scalper] Orphan prevention SELL succeeded`);
          } catch (e) {
            logPrintln(`[Scalper] Orphan prevention SELL failed: ${e} — tokens will resolve at market expiry`);
          }
          this.phase = { kind: "Cooldown", until: Math.floor(Date.now() / 1000) + 60, lastDirection: side };
          return;
        }
  ...
  private async verifyFillFromPolymarket(
    side: "BUY" | "SELL",
    tokenId: string,
    expectedPrice: number,
    expectedShares: number
  ): Promise<{ price: number; size: number; usdcSize?: number } | null> {
    const wallet = this.api.getWalletAddress();
    if (!wallet || !this.market) return null;
    ...
    try {
      ...
      const match = fills.find(f => {
        ...
        const recent = (now - f.timestamp * 1000) < 30_000;
      ...
      logPrintln(`[Scalper] Fill verification query failed: ${e}`);
    }
    return null;
  }

  private async reconcileBalanceWithPolymarket(): Promise<void> {
    ...
    try {
      ...
      logPrintln(`[Scalper] Balance reconciliation failed: ${e}`);
    }
  }

  private computeSafeSellShares(available: number, wanted: number): number {
    ...
    if (available >= wanted - 0.0001) return Math.floor(available * 1_000_000) / 1_000_000;
    return Math.floor(available * 1_000_000) / 1_000_000;
  }

  private emitTrade(data: {
    side: string; entryPrice: number; exitPrice: number; shares: number;
    costBasis: number; proceeds: number; pnlUsd: number; pnlPercent: number;
    exitReason: string;
  }): void {
    if (this.onTradeCallback) {
      try { this.onTradeCallback(data); } catch { /* ignore */ }
    }
  ...
  private isGeoBlockedError(e: unknown): boolean {
  ...
  private logPaperBuyBlock(
    ...
    logPrintln("");
    logPrintln(`---------- SIMÜLASYON | ÖRNEK ALIM #${tradeId} ----------`);
    logPrintln(`Piyasa (slug): ${slug}`);
    logPrintln(`Algoritma özeti: ${entryRuleSummary}`);
    logPrintln(`Taraf: ${side} | Pay (shares): ${shares} | Giriş fiyatı (sipariş defteri ask): $${price.toFixed(4)}`);
    logPrintln(`Tahmini maliyet (kağıt): $${cost.toFixed(2)}  (= ${shares} × $${price.toFixed(4)})`);
    logPrintln("Polymarket'e gerçek BUY emri gönderilmedi; sadece strateji + canlı orderbook ile kağıt pozisyon açıldı.");
    logPrintln("Çıkış: take-profit / stop-loss / periyot sonu kuralları bid fiyatına göre hesaplanır.");
    logPrintln("-----------------------------------------------------------");
    logPrintln("");
  }

  private resolveSellReferencePrice(tokenId: string): number | null {
    const bid = this.polyFeed.bestBids.get(tokenId) ?? 0;
  ...
  private async checkExitConditions(): Promise<void> {
    if (this.phase.kind !== "InPosition") return;
    ...
    if (heldSeconds <= rapidWindow && pnlPercent <= rapidThreshold) {
      logPrintln(`[Scalper] RAPID REVERSAL detected: ${(pnlPercent * 100).toFixed(1)}% in ${heldSeconds}s — emergency exit`);
  ...
  private async exitPositionInner(reason: string): Promise<void> {
    if (this.phase.kind !== "InPosition") return;
    const phase = this.phase;

    const exitRef =
      this.resolveSellReferencePrice(phase.tokenId) ?? this.polyFeed.bestBids.get(phase.tokenId) ?? phase.entryPrice;
    ...
    logPrintln(
      `[Scalper] SELL ${phase.side} ${phase.shares} shares @ $${currentBid.toFixed(4)} | ` +
      `Reason: ${reason} | P&L: $${totalPnl.toFixed(2)} (${pnlPercent >= 0 ? "+" : ""}${pnlPercent.toFixed(1)}%)`
    );

    let actualSellPrice: number | undefined;

    if (this.simulation) {
      const slug = this.market?.slug ?? "?";
      const reasonTr = this.describeExitReason(reason);
      logPrintln("");
      logPrintln(`---------- SIMÜLASYON | ÖRNEK SATIŞ / KAPANIŞ #${phase.tradeId} ----------`);
      logPrintln(`Piyasa: ${slug}`);
      logPrintln(`Giriş özeti: ${phase.entryRuleSummary}`);
      logPrintln(
        `Giriş (kağıt): ${phase.shares} pay × $${phase.entryPrice.toFixed(4)} = $${costBasis.toFixed(2)} maliyet`
      );
      logPrintln(
        `Çıkış (canlı bid): ${phase.shares} pay × $${currentBid.toFixed(4)} = $${proceeds.toFixed(2)} tahmini tahsilat`
      );
      logPrintln(`Çıkış nedeni: ${reasonTr} (${reason})`);
      logPrintln(
        `Brüt K/Z (ücret düşülmeden, bid/ask spread modeli): $${totalPnl.toFixed(2)} ` +
        `(${pnlPercent >= 0 ? "+" : ""}${pnlPercent.toFixed(2)}% giriş fiyatına göre)`
      );
      logPrintln(
        "Not: Girişte best ask, çıkışta best bid kullanılır. Gerçek hesapta taker ücreti, kısmi dolma ve slippage bu K/Z'yi değiştirir."
      );
      logPrintln("Polymarket'e gerçek SELL emri gönderilmedi.");
      logPrintln("------------------------------------------------------------------");
      logPrintln("");
      ...
      if (this.stuckSellShares == null) {
        try {
          ...
          logPrintln(`[Scalper] SELL balance query: available=${available.toFixed(6)}, phase.shares=${phase.shares}`);
          sellShares = this.computeSafeSellShares(available, phase.shares);
        } catch (e) {
          logPrintln(`[Scalper] SELL balance check failed (using phase.shares=${phase.shares}): ${e}`);
        }
      }

      if (sellShares > 0 && sellShares < this.minOrderSizeShares && this.minOrderSizeShares > 1) {
        logPrintln(
          `[Scalper] SELL shares ${sellShares.toFixed(4)} < minOrderSize ${this.minOrderSizeShares} — ` +
          `attempting sell anyway (CLOB may allow closing positions below min)`
        );
      }

      if (sellShares <= 0) {
        logPrintln("[Scalper] SELL skipped: zero balance. Position settled or dust.");
      ...
      let sold = false;
      for (let i = 0; i < maxAttempts; i++) {
        if (i > 0) await this.sleep(delays[i]);
        try {
          const result = await this.api.placeMarketOrder(phase.tokenId, sellShares, "SELL", "0.01", false, sellSlippage);
          if (result.actualPrice !== undefined) actualSellPrice = result.actualPrice;
          logPrintln(`[Scalper] REAL: Sell placed (${sellShares.toFixed(4)} shares @ $${(actualSellPrice ?? currentBid).toFixed(4)})${i > 0 ? ` attempt ${i + 1}` : ""}`);
          sold = true;
          break;
        } catch (e) {
          ...
          if (parsed) {
            ...
            if (reduced > 0 && reduced < sellShares) {
              logPrintln(
                `[Scalper] SELL size reduced: ${sellShares.toFixed(4)} → ${reduced.toFixed(4)} ` +
              ...
              if (sellShares < this.minOrderSizeShares && this.minOrderSizeShares > 1) {
                logPrintln(`[Scalper] Reduced size ${sellShares.toFixed(4)} < minOrderSize ${this.minOrderSizeShares} — trying sell anyway`);
              }
              continue;
            }
            if (reduced <= 0) {
              logPrintln("[Scalper] CLOB reports zero available balance. Abandoning position.");
          ...
          if (minMatch) {
            ...
            if (sellShares < clobMin) {
              logPrintln(
                `[Scalper] SELL impossible: ${sellShares.toFixed(4)} < CLOB min ${clobMin} shares. Abandoning.`
              );
          ...
          logPrintln(`[Scalper] SELL FAILED (attempt ${i + 1}/${maxAttempts}): ${e}`);
        }
      }
      ...
      if (!sold) {
        ...
        logPrintln(
          `[Scalper] SELL STUCK — retry #${this.sellRetryCount}/${this.MAX_SELL_STUCK_RETRIES} ` +
      ...
      let actualSellUsdcSize: number | undefined;
        ...
        for (let attempt = 0; attempt < SELL_SETTLE_DELAYS.length; attempt++) {
          ...
          try {
            ...
            if (delta > 0.001) {
              ...
              logPrintln(
                `[Scalper] SELL PROCEEDS (balance delta): $${delta.toFixed(2)} ` +
            ...
            if (attempt < SELL_SETTLE_DELAYS.length - 1) {
              logPrintln(`[Scalper] SELL settlement pending (delta=$${delta.toFixed(4)}) — waiting...`);
            }
          } catch (e) {
            logPrintln(`[Scalper] Post-sell balance check failed: ${e}`);
          }
        }
      }
      ...
      if (actualSellUsdcSize == null) {
        try {
          ...
          if (verified) {
            logPrintln(
              `[Scalper] SELL FILL VERIFIED via Polymarket: ${verified.size} shares @ $${verified.price.toFixed(4)} ` +
            ...
            logPrintln(`[Scalper] SELL fill not found in Polymarket history (using order limit price)`);
          }
        } catch (e) {
          logPrintln(`[Scalper] SELL fill verification failed: ${e}`);
        }
      }
      ...
      if (Math.abs(realPnl - totalPnl) > 0.001) {
        logPrintln(
          `[Scalper] P&L CORRECTED: estimated $${totalPnl.toFixed(2)} → actual $${realPnl.toFixed(2)} ` +
    ...
    logPrintln(
      `[Scalper] ${this.simulation ? "Kağıt özet" : "Özet"} — bugün: +$${this.dailyProfit.toFixed(2)} / -$${this.dailyLoss.toFixed(2)} | ` +
  ...
  private tryParseInsufficientBalance(errMsg: string): { balance: number; requested: number } | null {
    ...
    if (m1) {
      ...
      if (Number.isFinite(bal) && Number.isFinite(req)) {
        return { balance: bal / 1_000_000, requested: req / 1_000_000 };
      }
    }
    ...
    if (m2) {
      ...
      if (Number.isFinite(bal)) {
        return { balance: bal / 1_000_000, requested: Number.isFinite(req) ? req / 1_000_000 : (bal + 1) / 1_000_000 };
      }
    }
    return null;
  }

  private describeExitReason(reason: string): string {
    switch (reason) {
      case "take_profit":
  ...
  private appendSimulationTradeJsonl(record: Record<string, unknown>): void {
    if (!this.simulation) return;
    const file = path.join(process.cwd(), "simulation_trades.jsonl");
    try {
      fs.appendFileSync(file, JSON.stringify(record) + "\n", { encoding: "utf8" });
    } catch (e) {
      logPrintln(`[Scalper] simulation_trades.jsonl yazılamadı: ${e}`);
    }
  }

  private async abandonCurrentPosition(reason: string): Promise<void> {
    if (this.phase.kind !== "InPosition") return;
    const phase = this.phase;
    ...
    if (!this.simulation) {
      try {
        ...
        if (bal <= 0) {
          isPhantom = true;
          logPrintln(
            `[Scalper] PHANTOM TRADE DETECTED on abandon (${reason}): token balance is 0. ` +
            `No P&L recorded — the buy never actually filled.`
          );
        }
      } catch (e) {
        logPrintln(`[Scalper] Balance check in abandon failed: ${e}`);
      }
    }
    ...
    logPrintln(
      `[Scalper] POSITION ABANDONED (${reason}): ${phase.side} ${phase.shares} shares | ` +
  ...
  private async forceExitWithSell(): Promise<void> {
    if (this.phase.kind !== "InPosition") return;
    const phase = this.phase;
    logPrintln(`[Scalper] Force exit — position: ${phase.side} ${phase.shares} shares`);

    let actualExitPrice: number | undefined;
    let actualProceeds: number | undefined;

    if (!this.simulation) {
      let preSellBalance = 0;
      try { preSellBalance = await this.api.getCollateralBalance(); } catch { /* best-effort */ }
      ...
      try {
        ...
        if (sellShares > 0) {
          ...
          logPrintln(`[Scalper] Force exit SELL placed: ${sellShares} shares @ $${(actualExitPrice ?? 0).toFixed(4)} (limit)`);
          sold = true;
        }
      } catch (e) {
        logPrintln(`[Scalper] Force exit SELL failed: ${e}`);
      }
      ...
      if (sold && preSellBalance > 0) {
        ...
        try {
          ...
          if (delta > 0.001) {
            ...
            logPrintln(
              `[Scalper] Force exit PROCEEDS (balance delta): $${delta.toFixed(2)} ` +
      ...
      if (!sold && this.market) {
        try {
          logPrintln(`[Scalper] Attempting token redemption for ${this.market.conditionId.slice(0, 16)}...`);
          await this.api.redeemTokens(this.market.conditionId, phase.tokenId, phase.side);
          logPrintln(`[Scalper] Redemption successful`);
        } catch (e) {
          logPrintln(`[Scalper] Redemption failed (tokens may resolve at expiry): ${e}`);
        }
      }
    }
    ...
    logPrintln(
      `[Scalper] Force exit P&L: $${totalPnl.toFixed(2)} (cost=$${realCost.toFixed(2)} proceeds=$${realProceeds.toFixed(2)})`
    );
  ...
  private checkDailyReset(): void {
    ...
    if (today !== this.dailyResetDate) {
      logPrintln(`[Scalper] New day: ${today} | Yesterday P&L: +$${this.dailyProfit.toFixed(2)} / -$${this.dailyLoss.toFixed(2)}`);
      this.dailyLoss = 0;
      this.dailyProfit = 0;
  ...
  getStats(): { totalProfit: number; dailyProfit: number; dailyLoss: number; phase: string } {
    return {
      totalProfit: this.totalProfit,
  ...
  getSnapshot(): DashboardSnapshot {
    const pos = this.phase.kind === "InPosition" ? {
      side: this.phase.side as string,
